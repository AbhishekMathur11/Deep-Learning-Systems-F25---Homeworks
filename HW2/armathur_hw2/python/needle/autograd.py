...
class Op:
    ...
    def __call__(self, *args):
        ...
        return self.__class__.__call__(self, *args)
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for i in reverse_topo_order:
        ...
        incoming_grads = node_to_output_grads_list.get(i, [])
        if incoming_grads:
            adj_sum = sum_node_list(incoming_grads)
        else:
            adj_sum = adj_sum = Tensor(numpy.zeros_like(i.data), requires_grad=False)
        ...
        if i.op is not None:
            input_grads = i.op.gradient(adj_sum, i)

            if not isinstance(input_grads, (list, tuple)):
                input_grads = [input_grads]
            
            for inp, inp_grad in zip(i.inputs, input_grads):
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    for add in range(0, N):
        if node_list[add] not in visited:
            local_order = []
            topo_sort_dfs(node_list[add], visited, local_order)
            topo_order.extend(local_order)
