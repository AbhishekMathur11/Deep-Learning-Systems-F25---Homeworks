...
class NDArray:
    ...
    def reshape(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        neg_one_count = new_shape_list.count(-1)
    ...
    def broadcast_to(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        ### BEGIN YOUR SOLUTION
        old_shape = (1,) * (len(new_shape) - len(self.shape)) + self.shape
        old_strides = (0,) * (len(new_shape) - len(self.shape)) + self.strides
    ...
    def __getitem__(self, idxs: int | slice | tuple[int | slice, ...]) -> "NDArray":
        ...
        for i, slc in enumerate(slices):
            new_offset += slc.start * self.strides[i]
            dim_size = (slc.stop - slc.start + slc.step - 1) // slc.step
            
            if dim_size > 1 or slc.start == slc.stop - 1:
                new_shape.append(dim_size)
                new_strides.append(self.strides[i] * slc.step)
    ...
    def pad(self, axes: tuple[tuple[int, int], ...]) -> "NDArray":
        ...
        """
        ### BEGIN YOUR SOLUTION
        new_shape = tuple([self.shape[i] + axes[i][0] + axes[i][1] for i in range(len(axes))])
        
        out = NDArray.make(new_shape, device=self.device)
        out.fill(0)
        
        slices = tuple([slice(axes[i][0], axes[i][0] + self.shape[i]) for i in range(len(axes))])
        out[slices] = self
...
def dilate(a: NDArray, axes: tuple[int, ...], dilation: int) -> NDArray:
    """
    Dilate an array by inserting zeros between elements along specified axes.
    
    Args:
        a: Input array
        axes: Tuple of axes along which to dilate
        dilation: Number of zeros to insert between each element
    ...
    for axis in axes:
        new_shape[axis] = a.shape[axis] * (dilation + 1)
    ...
    for i in range(len(a.shape)):
        if i in axes:
            slices.append(slice(0, new_shape[i], dilation + 1))
        else:
...
def undilate(a: NDArray, axes: tuple[int, ...], dilation: int) -> NDArray:
    """
    Reverse of dilate - extract elements at dilated positions.
    
    Args:
        a: Input array (dilated)
        axes: Tuple of axes along which to undilate
    ...
    for i in range(len(a.shape)):
        if i in axes:
            slices.append(slice(0, a.shape[i], dilation + 1))
        else:
