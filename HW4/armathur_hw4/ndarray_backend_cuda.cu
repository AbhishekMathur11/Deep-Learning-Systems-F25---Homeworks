...
__device__ size_t index_to_position(size_t index, CudaVec shape, CudaVec strides, size_t offset) {
  size_t pos = offset;
  for (int i = shape.size - 1; i >= 0; i--) {
    pos += (index % shape.data[i]) * strides.data[i];
    index /= shape.data[i];
  }
  return pos;
}
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN SOLUTION
  if (gid < size) {
    size_t src_pos = index_to_position(gid, shape, strides, offset);
    out[gid] = a[src_pos];
  }
  /// END SOLUTION
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    size_t dst_pos = index_to_position(gid, shape, strides, offset);
    out[dst_pos] = a[gid];
  }
}
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  /// BEGIN SOLUTION
  // Use 2D thread blocks for matrix multiplication
