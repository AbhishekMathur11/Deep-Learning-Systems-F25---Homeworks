...
class BroadcastTo(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ...
        # Handle case where input has fewer dimensions
        ndim_diff = len(output_shape) - len(input_shape)
        if ndim_diff > 0:
            axes_to_sum.extend(range(ndim_diff))
        
        # Find dimensions that were broadcast from 1 to something larger
        for i in range(len(input_shape)):
            output_axis = i + ndim_diff
            if input_shape[i] == 1 and output_shape[output_axis] > 1:
                axes_to_sum.append(output_axis)
        
        # Sum over broadcast axes one at a time (backend only supports single axis reduction)
        grad = out_grad
        if axes_to_sum:
            # Sort in reverse order to sum from highest to lowest axis
            # This ensures axis indices remain valid after each reduction
...
class Summation(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ...
        # Determine which axes were summed
        if self.axes is None:
            # All axes were summed, result is scalar
            new_shape = [1] * len(input_shape)
        else:
            # Some axes were summed
            new_shape = list(input_shape)
            axes = self.axes if isinstance(self.axes, tuple) else (self.axes,)
            for axis in axes:
                new_shape[axis] = 1
        
        # Reshape gradient to match broadcasted shape
...
class Stack(TensorOp):
    ...
    def compute(self, args: TensorTuple) -> Tensor:
        ...
        # Create new shape with axis inserted
        new_shape = list(shape)
        new_shape.insert(self.axis, n)
        
        # Allocate output array - use the device from the first array
        device = arrays[0].device
        out = device.empty(tuple(new_shape), dtype=arrays[0].dtype)
        
        # Create slices for each position along new axis
