...
class MultiHeadAttention(Module):
    ...
    def matmul(self, a, b, *, transpose_b=False):
        """
        Batched matrix multiplication implemented via per-head matmuls.
        Uses core ops.matmul kernels to avoid backend-specific broadcast bugs.
        """
        ...
        slices = []
        for idx in np.ndindex(*batch_shape) if batch_shape else [()]:
            left_matrix = a[idx]
            right_matrix = b[idx]
            if transpose_b:
                right_matrix = ops.transpose(right_matrix)
            slices.append(ops.matmul(left_matrix, right_matrix))

        stacked = ops.stack(tuple(slices), axis=0)
        return ops.reshape(
            stacked,
            (*batch_shape, left_m, right_n)
        )
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        qk = self.matmul(q, k, transpose_b=True) / math.sqrt(q_dim)
...
class AttentionLayer(Module):
    ...
    def forward(
        ...
        q_w = self.q_projection(q_norm)
        k_w = self.k_projection(k_norm)
        v_w = self.v_projection(v_norm)
